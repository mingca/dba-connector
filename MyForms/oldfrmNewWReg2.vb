...
#Region "Load"
    ...
    'LOAD
    Private Sub oldfrmNewWReg2_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) _
        Handles Me.Load
...
FormSetup:
    ...
    'let calling form call this so is done before assign value
...
ReleaseForm:
    ...
    Public Function UpdateDB() As Boolean
        MouseWait()
        modPopup.InsertRegistration(Me.fldOrderNum.Text, Me.cboEvent.SelectedValue, Me.cboRegistrant.SelectedValue, Me.DtRegister.Text)
        MouseDefault()
    End Function
    ...
    Private Function CheckRequired() As Control()
        ...
        If Me.DtRegister.Text = String.Empty Then
            ctl = Me.DtRegister
            Me.ErrorProvider1.SetError(ctl, "please enter a " & IsNull(ctl.Tag, ctl.Name))
            arCtls(i) = ctl
            i = i + 1
...
#Region "Validating"
    ...
    'date validation
    Private Sub DtRegister_Validating(sender As Object, e As System.ComponentModel.CancelEventArgs) _
        Handles DtRegister.Validating
...
#Region "General"
    ...
    Private Sub btnNewOrder_Click(sender As System.Object, e As System.EventArgs) Handles btnNewOrder.Click
        'Try
        '    ThisReg.OrderID = CType(Inpt, Integer)
    ...
    Private Sub btnRefreshContacts_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) _
        ...
        'Dim i As Integer = Me.fldContactNum.Text
        ...
        '  If i > 0 Then
        ' Dim s As String = "ContactID = '" & Me.fldContactNum.Text & "'"
        '  Dim t As String
        'Me.cboRegistrant.SelectedValue
        ' Try
        ' Dim r As DataRow() = dtRegistrant.Select(s)
        ' t = r(0).Item("ContactOrgCity")
        ' Catch ex As System.Exception
        ' ' modGlobalVar.Msg(ex.Message)
        ' End Try

        'If LoadRegistrantDD(True) Then
        ' If s Is String.Empty Then
        ' Exit Sub
        ' End If
        'Me.isLoaded = False
        'Me.cboRegistrant.SuspendLayout()
        '   Me.cboRegistrant.SelectedIndex = Me.cboRegistrant.f(s)
        'Try
        ' Me.cboRegistrant.SelectedIndex = Me.cboRegistrant.FindStringExact(t)
        ' Catch ex As Exception
        ' modGlobalVar.Msg(ex.Message, , "can't select original value")
        ' End Try
        '
        '        Me.cboRegistrant.ResumeLayout()
        '        Me.isLoaded = True
        '        End If
        '        s = Nothing
        't = Nothing
        '       Else
        '     LoadRegistrantDD(True)
        '      End If
        ' r = Nothing
    End Sub
