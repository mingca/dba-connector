...
#Region "Load"
    'LOAD
    Private Sub frmMainTMGI_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
...
FormatFields:
            ...
            If pnl.GetType.ToString = "System.Windows.Forms.Panel" Then
                For Each ctl In pnl.Controls  'Me.pnlApplication.Controls
                    If ctl.Name.Substring(0, 2) = "dt" Then
                        '  FormatDateFld(ctl, "text", Me.DsMainGrantTMGI1, "tblGrantTMGI." & ctl.Name.Substring(2, Len(ctl.Name) - 2) & "dt")
        ...
        mainTbl = Me.DsMainGrantTMGI.tblGrantTMGI
...
Update:
    ...
    Protected Sub miClose_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) _
            ...
            Try
                For Each c As DataColumn In mainTbl.Columns
                    ...
                    Else
                        ...
                        Else
                            '   modGlobalVar.Msg( c.ColumnName,"current: '" & dr.Item(c, DataRowVersion.Current) & "'" & NextLine & "Proposed: '" & dr.Item(c, DataRowVersion.Proposed) & "'" & NextLine & "Original: '" & dr.Item(c, DataRowVersion.Original) & "'", 0,information)
                            Mystery = True
                            Exit For
                        End If
                    End If
...
ReleaseForm:
        If bCancelClose = False Then   'user OKs close form
            ClassOpenForms.frmMainTMGI = Nothing 'reset global var
        Else
        End If
    ...
    Public Sub UpdateDB(ByVal How As String)
        ...
        Me.DsMainGrantTMGI.EnforceConstraints = False
...
BeginTransaction:
        ...
        Try
            ...
            Me.tblGrantTMGITableAdapter.Update(Me.DsMainGrantTMGI.tblGrantTMGI)
            '  myTransaction.Commit()
            bCancelClose = False
        Catch dbcex As DBConcurrencyException
            modGlobalVar.Msg("ERROR: Update Failed", "someone else has changed this Recommendation", MessageBoxButtons.OK, MessageBoxIcon.Error)
            'TODO put code here to capture changes and/or override
            '  myTransaction.Rollback()
            bCancelClose = True
        Catch eUpdate As System.Exception
            modGlobalVar.Msg("ERROR: updated", eUpdate.Message & NextLine & eUpdate.GetType.ToString, MessageBoxButtons.OK, MessageBoxIcon.Error)
            bCancelClose = True
            '  myTransaction.Rollback()
        Finally
            sc.Close()
...
#Region "Edit buttons"
    ...
    Private Sub btnDelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) _
        ...
        If modGlobalVar.Msg("CONFIRM DELETE", "This TMGI application will be marked for deletion and the window closed.", MessageBoxButtons.YesNoCancel, MessageBoxIcon.Question) = DialogResult.Yes Then
    ...
    Private Sub migotoGrant_click(ByVal sender As System.Object, ByVal e As System.EventArgs) _
        ...
        If IsNull(mainTbl.Rows(0).Item("GrantIDTxt"), "a").ToString = "a" Then  'grant does not exist yet
            If CType(IsNull(Me.dtGrantAppRecd.Text, "1/1/1911"), Date) > CType("1/1/2012", Date) Then
                modPopup.NewGrant(fldOrgID.Text, "Technology Ministry Grant", True, "TECHNOLOGY & MINISTRY GRANT", "TMGI", Me.fldGINum.Text, fldCaseID.Text, Me.dtGrantAppRecd.Text, IsNull(Me.cboPD.SelectedValue, 0), IsNull(Me.cboClergyLeader.SelectedValue, 0), usr)
            ...
            End If
        Else
            modGlobalVar.OpenMainGrant(mainTbl.Rows(0).Item("GrantIDTxt"), "TMGI Grant", Me.fldGotoOrg.Text, Me.fldOrgID.Text)
        End If
        'End If
    End Sub

    'SET DEADLINE DATE
    Private Sub cboClassGroup_SelectionChangeCommitted(ByVal sender As System.Object, ByVal e As System.EventArgs) _
        Handles cboClassGroup.SelectionChangeCommitted
...
#End Region 'general
    ...
    Private Sub btnHelp_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) _
        ...
        modGlobalVar.Msg("TMGI HELP", "to create a grant, go to the Case Detail window.", MessageBoxButtons.OK, MessageBoxIcon.Information)
    End Sub
